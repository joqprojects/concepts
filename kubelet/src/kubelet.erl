...
%% Records
%% --------------------------------------------------------------------
-record(state, {lSock,max_instances,active_workers,workers,service_list}).
%% --------------------------------------------------------------------
...
-export([boot/1,app_start/3,upgrade/0,upgrade/1]).
...
upgrade(ServiceId,Vsn)-> gen_server:cast(?MODULE, {ServiceId,Vsn}).
...
%% Returns: non
%% --------------------------------------------------------------------
handle_call({start_service,ServiceId,Vsn}, From, State) ->
    case lists:keymember(ServiceId,1,State#state.service_list) of
	true->
...
handle_call({stop_service,ServiceId}, From, State) ->
    case lists:keymember(ServiceId,1,State#state.service_list) of
	false->
	    {error,[?MODULE,?LINE,'service nexists',ServiceId,Vsn]};
	true->
...
%%          {stop, Reason, State}            (terminate/2 is called)
%% --------------------------------------------------------------------
handle_cast({upgrade,Glurk}, State) ->
	    % get tar file from SW repositroy
	    % create service_info record
	    % create temp dir 
	    % untar files 
...
l_upgrade(State)->
    ...
    ok=sys:change_code(?MODULE,?MODULE,"Old_Vsn",[]),
